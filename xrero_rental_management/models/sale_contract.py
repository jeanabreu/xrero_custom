...
class PropertyVendor(models.Model):
    ...
    booking_item_id = fields.Many2one('product.product',
                                      string="Booking Item",
                                      default=lambda self: self.env.ref('xrero_rental_management.property_product_2',
                                                                        raise_if_not_found=False))
    broker_item_id = fields.Many2one('product.product',
                                     string="Broker Item",
                                     default=lambda self: self.env.ref('xrero_rental_management.property_product_1').id)
    ...
    def sale_recurring_invoice(self):
        ...
        ).get_param('xrero_rental_management.sale_reminder_days')
        ...
        for data in sale_invoice:
            ...
            ).get_param('xrero_rental_management.invoice_post_type')
    ...
    def send_sold_mail(self):
        mail_template = self.env.ref(
            'xrero_rental_management.property_sold_mail_template')
        if mail_template:
    ...
    def action_book_invoice(self):
        mail_template = self.env.ref(
            'xrero_rental_management.property_book_mail_template')
        if mail_template:
            mail_template.send_mail(self.id, force_send=True)
        record = {
            'product_id': self.env.ref('xrero_rental_management.property_product_1').id,
...
class SaleInvoice(models.Model):
    ...
    def action_send_sale_invoice(self, invoice_id):
        mail_template = self.env.ref(
            'xrero_rental_management.sale_invoice_payment_mail_template')
        if mail_template:
