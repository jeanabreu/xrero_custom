...
class TenancyDetails(models.Model):
    ...
    def action_active_contract(self):
        invoice_post_type = self.env['ir.config_parameter'].sudo().get_param(
            'xrero_rental_management.invoice_post_type')
        invoice_lines = []
    ...
    def action_send_active_contract(self):
        mail_template = self.env.ref(
            'xrero_rental_management.active_contract_mail_template')
        if mail_template:
    ...
    def action_send_tenancy_reminder(self):
        mail_template = self.env.ref(
            'xrero_rental_management.tenancy_reminder_mail_template')
        if mail_template:
            mail_template.send_mail(self.id, force_send=True)
    ...
    def tenancy_recurring_invoice(self):
        ...
        ).get_param('xrero_rental_management.reminder_days')
