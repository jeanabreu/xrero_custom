...
class StockMove(models.Model):
    ...
    #     vals['branch_id'] = self.group_id.sale_id.branch_id.id if not self.env.company.branch_not_used else False
    #     return vals
    ...
    def _create_account_move_line(self, credit_account_id, debit_account_id, journal_id, qty, description, svl_id, cost):
        ...
        if move_lines:
            ...
            new_account_move = AccountMove.sudo().create({
                ...
                'move_type': 'entry',
                'branch_id': self.picking_id.branch_id.id if not self.env.company.branch_not_used else False
            })
    ...
    def _generate_valuation_lines_data(self, partner_id, qty, debit_value, credit_value, debit_account_id, credit_account_id, description):
        ...
        branch_id = False
        if self.branch_id and not self.env.company.branch_not_used:
            branch_id = self.branch_id.id
        elif self.env.user.branch_id and not self.env.company.branch_not_used:
            branch_id = self.env.user.branch_id.id

        for res in result:
            result[res].update({'branch_id' : branch_id if not self.env.company.branch_not_used else False})
