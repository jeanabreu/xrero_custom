...
class SaleOrder(models.Model):
    ...
    def action_view_work_order(self):
        ...
        result = mod_obj._xmlid_lookup("%s.%s" % ('xrero_car_repair_industry', 'action_fleet_workorder_tree_view'))
        id = result and result[1] or False
        result = act_obj.browse(id).read()[0]
        res = mod_obj._xmlid_lookup("%s.%s" % ('xrero_car_repair_industry', 'view_fleet_workorder_form'))
            ...
            if vals.get('state') == 'posted':
                for move in self:
                    ...
                    if sale_obj  and sale_obj.workorder_id and sale_obj.workorder_id.fleet_repair_id:
                        ...
                        template_id = self.env.ref('xrero_car_repair_industry.car_repair_service_done').id
